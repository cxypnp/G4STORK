...
G4ReactionProduct * G4NeutronHPLabAngularEnergy::Sample(G4double anEnergy, G4double massCode, G4double )
{
   ...
   {
       G4double **running = new G4double *[2];
       G4double cosThTemp[2], secEnergyTemp[2], currentMeanEnergyTemp[2];

       for(G4int inEIdx=-1; inEIdx<1; inEIdx++)
       {
            // integrate the prob for each costh, and select theta.
            running[inEIdx+1] = new G4double [nCosTh[it+inEIdx]];
            running[inEIdx+1][0]=0;
            for(i=0;i<nCosTh[it+inEIdx]; i++)
            {
                if(i!=0) running[inEIdx+1][i] = running[inEIdx+1][i-1];
                running[inEIdx+1][i]+=theData[it+inEIdx][i].GetIntegral(); // Does interpolated integral.
            }
            G4double random = running[inEIdx+1][nCosTh[it+inEIdx]-1]*G4UniformRand();
            G4int ith(0);
            for(i=0;i<nCosTh[it+inEIdx]; i++)
            {
                ith = i;
                if(random<running[inEIdx+1][i]) break;
            }
            //080807
            //if ( ith == 0 || ith == nCosTh[it]-1 ) //ith marks the angluar bin
            if ( ith == 0 ) //ith marks the angluar bin
            {
                cosThTemp[inEIdx+1] = theData[it+inEIdx][ith].GetLabel();
            }
            else
            {
                //080808
                //G4double x1 = theData[it][ith-1].GetIntegral();
                //G4double x2 = theData[it][ith].GetIntegral();
                G4double x1 = running[inEIdx+1] [ ith-1 ];
                G4double x2 = running[inEIdx+1] [ ith ];
                G4double x = random;
                G4double y1 = theData[it+inEIdx][ith-1].GetLabel();
                G4double y2 = theData[it+inEIdx][ith].GetLabel();
                cosThTemp[inEIdx+1] = theInt.Interpolate(theSecondManager[it+inEIdx].GetInverseScheme(ith),
                                          x, x1, x2, y1, y2);
            }
       }
       ...
       for(G4int inEIdx=-1; inEIdx<1; inEIdx++)
       {
            ...
            {
                ith = i;
                if(cosTh<theData[it+inEIdx][ith].GetLabel()) break;
            }
            G4double x1 = running[inEIdx+1] [ ith-1 ];
            G4double x2 = running[inEIdx+1] [ ith ];
            G4double y1 = theData[it+inEIdx][ith-1].GetLabel();
            ...
            if ( ith == 0 ) //ith marks the angluar bin
            {
                secEnergyTemp[inEIdx+1] = theData[it][ith].Sample();
                currentMeanEnergyTemp[inEIdx+1] = theData[it][ith].GetMeanX();
            }
            else
            {
                G4NeutronHPVector theBuff1;
                ...
                G4NeutronHPVector theStore;
                theStore.Merge(&theBuff1, &theBuff2);
                secEnergyTemp[inEIdx+1] = theStore.Sample();
                currentMeanEnergyTemp[inEIdx+1] = theStore.GetMeanX();
            }
            delete [] running[inEIdx+1];
       }
       secEnergy = theInt.Lin(anEnergy,theEnergies[it-1],theEnergies[it],secEnergyTemp[0],secEnergyTemp[1]);
       currentMeanEnergy = theInt.Lin(anEnergy,theEnergies[it-1],theEnergies[it],currentMeanEnergyTemp[0],currentMeanEnergyTemp[1]);
       delete [] running;
