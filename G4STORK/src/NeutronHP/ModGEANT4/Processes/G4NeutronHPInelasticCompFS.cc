...
//
#include "../../../../include/NeutronHP/ModGEANT4/Processes/G4NeutronHPInelasticCompFS.hh"
...
// prepare energy in target rest frame
    ...
    if(!runLikeMCNP->GetFlag())
    {
        boosted.Lorentz(theNeutron, -(std::sqrt((theTrack.GetMaterial()->GetTemperature()-fsTemp)/theTrack.GetMaterial()->GetTemperature()))*theTarget);
...
// Type B2 make difference in total energy.
 ...
 bool G4NeutronHPInelasticCompFS::ExtractTemp(G4String name, G4double &temp)
{
    bool check = false;
    std::stringstream ss;
