...
//Modified by Chengxi Yang. Save the Delayed Neutrons to Delayed instead of survivors
...
G4bool StorkNeutronSD::ProcessHits(G4Step *aStep, G4TouchableHistory*)
{
    ...
    StorkIsoEnerScore *isoEnerScore;
...
//            (*itr)->SetKineticEnergy(2);
            if((*itr)->GetTrackStatus()!=fKillTrackAndSecondaries)
            {
                ...
                {
                    if(isoEnerScore->Ready())
                    {
                        isoEnerScore->ScoreParticle(preStepPoint->GetKineticEnergy(), (*itr)->GetKineticEnergy(),2);
                    }
        ...
        // changed fnenergy to collect the energy to the poststeppoint instead of the presteppoint
...
//            (*itr)->SetKineticEnergy(2);
            if((*itr)->GetTrackStatus()!=fKillTrackAndSecondaries)
            {
                ...
                if((*itr)->GetDefinition() == G4Neutron::NeutronDefinition())
                {
                    if(isoEnerScore->Ready())
                    {
                        isoEnerScore->ScoreParticle(preStepPoint->GetKineticEnergy(), (*itr)->GetKineticEnergy(),3);
                    }
                    if(nMulti)
                    {
                        nProd++;
	...
	else if(hitProcess == "StorkHadronElastic")
	{
        ...
        if(isoEnerScore->Ready())
        {
            isoEnerScore->ScoreParticle(preStepPoint->GetKineticEnergy(), aTrack->GetKineticEnergy(),0);
        }
