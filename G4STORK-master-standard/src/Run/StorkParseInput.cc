...
: isMaster(master)
{
    ...
    scoreIsoCheck = false;
    scoreReacCheck = false;
    scoreIsoReacCheck = false;
    scoreIsoEnerCheck = false;

    NeutronHPModCheck *checkNeutHPMod;
    checkNeutHPMod->SetFlag(false);
    neutronHPMod=false;
...
G4bool StorkParseInput::ReadInputFile(G4String filename)
{
    ...
    while(infile.good())
    {
		...
		// Use Modified NeutronHP
		else if(keyWord=="NEUTRONHP_MOD")
		{
            G4String tempStr;
			infile >> tempStr;

			if(tempStr=="1"||tempStr=="true"||tempStr=="True")
			{
                NeutronHPModCheck *checkNeutHPMod;
                checkNeutHPMod->SetFlag(true);
                neutronHPMod=true;
            }
		}

		else if(keyWord=="RUN_LIKE_MCNP")
		{
            G4String tempStr;
			infile >> tempStr;
		...
		//Scoring
		else if(keyWord=="SCORE_ISO")
		{
			infile >> scoreIsoDir;
			scoreIsoCheck=true;
		}
		else if(keyWord=="SCORE_REAC")
		{
			infile >> scoreReacDir;
			scoreReacCheck=true;
		}
		else if(keyWord=="SCORE_ISO_REAC")
		{
			infile >> scoreIsoReacDir;
			scoreIsoReacCheck=true;
		}
		else if(keyWord=="SCORE_ISO_ENERGY")
		{
			infile >> scoreIsoEnerDir;
			scoreIsoEnerCheck=true;
		}
